...
class Augmentation:
    def __init__(self,route,Image,Xml,n):
        self.route=route
    ...
    def cut_image_mask(self,crop_h=800,crop_w=1000,gap=100):
        for h in range(0,self.height,gap):
            for w in range(0,self.width,gap):
        ...
        if len(Objects)>=min_object:      
            ...
            for o_idx in range(len(object)):
                ...
                if cnt<=len(Objects): #cropimage 안에 object 수 
                    object[o_idx].find("name").text=self.object_name_list[o_idx]
...
for idx in range(len(Image_list)):
    ...
    A=Augmentation(route,Image,Xml,0)#0은 저장할때 카운트
    A.xml_parsing()
